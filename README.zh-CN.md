...
需要能操作浏览器、读取页面和查看图片的 Agent 环境，**目前在 Codex 中验证过**。先连接浏览器，确认目标平台可以访问，需要时完成登录。toddle 提供技能和媒体脚本，浏览器连接与平台账号由你的环境提供。
...
Windows 默认目录是 `%USERPROFILE%\.codex\skills\toddle-skill`。已有同名目录时先检查旧版本，保留本机修改。其他 Agent 需要具备同等的浏览器、图片和文件工具，本项目尚未验证其兼容性。
...
AI 先从页面、实际画面和可读字幕开始。需要更多证据时，它会处理媒体下载、取帧或转写，让你可以直接开始浏览，无需先研究视频工具链。
...
**你可以纠正它对兴趣的理解。** 平台推荐、你本人的行为、你的明确反馈，分别作为不同来源的证据；AI 自己的点击不算你的偏好。它通过本地笔记调整后续选片，不会重新训练底层模型，也不声称读取了平台内部画像。具体规则见[兴趣记录](references/interests.md)。
...
| 脚本 | 用途 |
|---|---|
| [`doctor.py`](scripts/doctor.py) | 检查可复用的 Python 环境、包版本、FFmpeg 和本地模型候选。 |
...
默认数据目录是 `${XDG_DATA_HOME:-~/.local/share}/toddle-skill/`：
...
模型、下载的媒体和私人笔记放在仓库之外，本项目不会自动发布这些文件。用于分析的页面文字、图片和转写会进入所用 AI 的上下文，数据处理方式取决于对应产品与账户设置。
